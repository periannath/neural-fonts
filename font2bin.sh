...
if [ "$#" -ne 2 ]; then
  echo "Usage : ./font2img.sh src_font dst_font"
else
  if [[ -f fonts/$1 && -f fonts/$2 ]]; then
    FONT1=$(echo $1 | awk -F'.' '{print $(NF-1)}')
    FONT2=$(echo $2 | awk -F'.' '{print $(NF-1)}')
    FOLDER=${FONT1}2${FONT2}-order
    if [ ! -d image/$FOLDER ]; then
      echo "font2img.py $FONT1 $FONT2"
      mkdir -p image/$FOLDER
      python font2img.py --src_font=fonts/$1 --dst_font=fonts/$2 --charset=KR --sample_count=1000 --sample_dir=image/$FOLDER --label=0 --filter=1 --shuffle=0
    fi
