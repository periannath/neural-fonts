...
if [ "$#" -eq 0 ]; then
  ...
  if [ -f fonts/$1 ]; then
	...
	do
	  ...
	  python font2img.py --src_font=fonts/$SRCFONT --dst_font=$TARGETFONT --sample_count=2000 --sample_dir=image/$FOLDER --label=${INDEX} --filter=1 --shuffle=1 --x_offset=53 --y_offset=23
