...
if [ "$#" -ne 1 ]; then
  ...
  if [[ -d binary/$1 ]]; then
    ...
    python train.py --experiment_dir=binary/$1 --experiment_id=0 --batch_size=16 --lr=0.001 --epoch=120 --sample_steps=100 --schedule=30  --L1_penalty=500 --Lconst_penalty=1000 --freeze_encoder=1
  else
